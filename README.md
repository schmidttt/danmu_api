...
| 检查结果 | 自动处理 | 需要的人工操作 |
| --- | --- | --- |
| 上游无新提交 | 任务成功结束，不创建 PR | 无 |
| 上游有新提交 | 创建独立 Draft PR，触发 Node 与 Docker 验证，由 Vercel 生成 Preview | 按 [`DOWNSTREAM_PATCHES.md`](DOWNSTREAM_PATCHES.md) 审查补丁，验证 Preview，确认后再合并 |
| 上游合并冲突或验证失败 | 任务失败并保持 `main` 不变 | 查看运行摘要，在提案分支上人工适配 |

定时任务默认每日约北京时间 05:17 检查，但 GitHub Actions 的排队可能使实际开始时间延后，它不是实时同步。需要立即检查时，在 [Actions 的 Fork Sync Pull Request 页面](https://github.com/schmidttt/danmu_api/actions/workflows/sync_fork.yml) 手动运行即可；手动运行也遵守相同的 Draft PR 和生产保护边界。
...
- **API 接口**：
  ...
  - `POST /api/v2/favorite/add`：新增收藏。手动匹配测试使用 `{ "keyword": "火影忍者" }` 保存搜索关键词及整组搜索结果；同时兼容 `{ "fileName": "火影忍者 S01E01" }`。serverless 未配置 Redis 时，新增、刷新和删除接口返回 `503`，避免写入冷启动后会丢失的内存收藏。
...
| TENCENT_DANMU_DEADLINE_MS | 【可选】腾讯弹幕分片的整体等待截止时间（毫秒），默认为`8000`。到达截止时间后会取消仍未完成的慢分片，并返回已经成功获取的分片，避免单个异常分片拖慢整集；可配置范围为`1000-30000`。       |
